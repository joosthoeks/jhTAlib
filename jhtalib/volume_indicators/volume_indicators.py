...
def CMF(df, n, high='High', low='Low', close='Close', volume='Volume'):
    """
    Chaikin Money Flow
    Returns: list of floats = jhta.CMF(df, n, price='Close')
    ...
    for i in range(len(df[close])):
        if i + 1 < n:
            ...
            formula_list = []
            for i2 in range(len(df[close][start:end])):
                h = df[high][start:end][i2]
